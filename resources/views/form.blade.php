...
@section('container')
    ...
    <div class="container mt-5">
        <div class="row mt-4">
            <div class="col lg-6">
                ...
                <input type="text" class="form-control mb-3 @error('title') is-invalid @enderror" id="title" name="title" placeholder="Research Title" value="{{ old('title') }}">
                @error('title')
                ...
                <textarea class="form-control mb-3" id="abstract" rows="13" name="abstract">{{ old('abstract') }}</textarea>
            </div>

            <div class="col lg-6">

            <label for="inputDospem" class="form-label fw-bold @if ($errors->has('dosenName')) is-invalid @endif" style="color: #002147">Advisors</label>
            ...
            <div id="advisorInputs">
                ...
                <div class="row">
                    <div class="col">
                        <input type="text" class="form-control mb-3" id="dosenFirstName{{ $i+1 }}" name="dosenFirstName[]" placeholder="First Name" value="{{ is_array(old('dosenFirstName')) ? old('dosenFirstName')[$i] : '' }}">
                    </div>
                    <div class="col">
                        <input type="text" class="form-control mb-3" id="dosenLastName{{ $i+1 }}" name="dosenLastName[]" placeholder="Last Name" value="{{ is_array(old('dosenFirstName')) ? old('dosenLastName')[$i] : '' }}">
                    </div>
                </div>
                @endfor
            </div>
            @if ($errors->has('dosenName'))
                <div class="invalid-feedback">
                    {{ $errors->first('dosenName') }}
                </div>
            @endif
            ...
            <div style="color: #002147">
                ...
                <div class="form-check form-check-inline mb-3 mt-2">
                    <input class="form-check-input" type="radio" name="lab_id" id="lab_id" value="{{ $lab->id }}" {{ old('lab_id') == $lab->id ? 'checked' : '' }}>
            ...
            <div style="color: #002147">
                ...
                <div class="form-check form-check-inline mb-3 mt-2">
                    <input class="form-check-input" type="radio" name="jenjang_id" id="jenjang_id" value="{{ $jenjang->id }}" {{ old('jenjang_id') == $jenjang->id ? 'checked' : '' }}>
            ...
            <input type="text" class="form-control mb-3" placeholder="Research Product URL" id="url" name="url" value="{{ old('url') }}">
    ...
    document.getElementById('addAdvisorButton').addEventListener('click', function() {
        var totalAdvisors = document.querySelector('#advisorInputs').children.length;
        if (totalAdvisors < 3) {
